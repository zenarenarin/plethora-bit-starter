...
BASE_DIR = 'D:/Scroller/plethora-bit-starter'
BOOK_DIR = os.path.join(BASE_DIR, 'coloringbook')
OUT_DIR  = os.path.join(BASE_DIR, 'example_bits')

BITS = [
    {
        'file':  'FishOnPage.png',
        'title': 'Fish Coloring',
        'desc':  'Color a fisherman and jumping fish scene',
        'out':   'fish_coloring.js',
    },
    {
        'file':  'canva-black-and-white-illustrated-coloring-page-dinosaur-a4-fJCeJ297uD8.png',
        'title': 'Dinosaur Coloring',
        'desc':  'Color a cute kawaii dinosaur scene',
        'out':   'dinosaur_coloring.js',
    },
    {
        'file':  'il_570xN.1553473561_5e0w.png',
        'title': 'Mandala Coloring',
        'desc':  'Color an intricate floral mandala design',
        'out':   'mandala_coloring.js',
    },
    {
        'file':  'liberty.png',
        'title': 'Liberty Coloring',
        'desc':  'Color the Statue of Liberty and city skyline',
        'out':   'liberty_coloring.js',
    },
    {
        'file':  'lotofomushrooms.png',
        'title': 'Mushroom Forest Coloring',
        'desc':  'Color a detailed mushroom forest scene',
        'out':   'mushroom_forest_coloring.js',
    },
]

TEMPLATE = r"""window.scrollerApp = {
  meta: {
    title: '___TITLE___',
    author: 'YourUsername',
...
for bit in BITS:
    img_path = os.path.join(BOOK_DIR, bit['file'])
    with open(img_path, 'rb') as f:
        b64 = base64.b64encode(f.read()).decode('ascii')

    js = TEMPLATE
    js = js.replace('___TITLE___', bit['title'])
    js = js.replace('___DESC___', bit['desc'])
    js = js.replace('___B64___', b64)

    src_path = os.path.join(BASE_DIR, 'src', 'index.js')
    with open(src_path, 'w') as f:
        f.write(js)

    print(f"Written src/index.js for {bit['title']} (b64 size: {len(b64):,})")

    ret = os.system(f'cd /d {BASE_DIR} && npm run build 2>&1')
    if ret != 0:
        print(f"  ERROR: build failed for {bit['title']}")
        continue

    out_path = os.path.join(OUT_DIR, bit['out'])
    shutil.copy(src_path, out_path)
    print(f"  Saved to example_bits/{bit['out']}")
